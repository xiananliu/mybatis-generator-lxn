    ...
    <#list attrs as attr>
        ...
        <if test="set.${attr.propertiesName} != null<#if attr.javaTypeName=="String"> and set.${attr.propertiesName}!=''</#if>">
            ${sense}${attr.columnName}${sense} = ${"#\{set."}${attr.propertiesName}}::${attr.typeName},
        </if>
        </#if>
    </#list>
        </trim>
        <trim prefix="where" suffixOverrides="and | or">
            ...
            <if test="where.${attr.propertiesName}St !=null">
                ${sense}${attr.columnName}${sense} >= ${"#\{where."}${attr.propertiesName}St}::${attr.typeName} and
            </if>
            <if test="where.${attr.propertiesName}Ed!=null">
                ${sense}${attr.columnName}${sense} &lt;= ${"#\{where."}${attr.propertiesName}Ed}::${attr.typeName} and
            </if>
            </#if>
